...
func GenFiles(weight int, path string) {
	...
	for i := 1; i <= weight; i++ {
		...
		if err := utils.WriteFile(paragraph, fmt.Sprintf("%v/Text%d.txt", path, i)); err != nil {
			log.Fatal(err)
		}
	}
}
...
func GenFilesAndFolders(weight int, path string) {
	...
	for i := 1; i <= weight; i++ {
		...
		if i%3 == 0 {
			...
			if err := utils.WriteFile(paragraph, fmt.Sprintf("%s/Text%d.txt", filePath, i)); err != nil {
				log.Fatal(err)
			}
			continue
		}
		...
		if i%13 == 0 {
			...
			} else if err := utils.WriteFile(paragraph, fmt.Sprintf("%s/Text%d.txt", filePath, i)); err != nil {
				log.Fatal(err)
			}
			continue
		}
